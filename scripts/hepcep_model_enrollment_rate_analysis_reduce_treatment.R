...
for (d in dirs){
  ...
  if (!file.exists(path)){
    ...
    tryCatch({
      ...
      table$reduced_treatment_enrollment_per_PY <- props[Name=="reduced_treatment_enrollment_per_PY"]$Value
...
incidenceYear <- data[Year %in% startYear:endYear, .(incidence=1000*sum(incidence_daily_chronic/(population_ALL-infected_ALL))), 
                    by=list(Year,treatment_enrollment_per_PY, reduced_treatment_enrollment_per_PY, 
...
incidenceSummarySubset <- incidenceSummary[treatment_enrollment_per_PY != 0]
...
treatedYear <- data[Year %in% startYear:endYear, .(treated=sum(treatment_recruited_daily)), 
                  by=list(Year,reduced_treatment_enrollment_per_PY, treatment_enrollment_per_PY,
                          treatment_nonadherence,max_num_daa_treatments,run)]
